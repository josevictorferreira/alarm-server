...
module Parsers
  ...
  MessageData = Data.define(:message_id, :device_id, :type, :time, :notification_message, :parsed_data, :raw_message) do
    def initialize(message_id:, device_id:, type:, time:, notification_message:, parsed_data:, raw_message:)
      super(
  ...
  NotificationData = Data.define(:title, :device_name, :device_address, :device_channel, :formatted_time, :status) do
    def initialize(title:, device_name:, device_address:, device_channel:, formatted_time:, status:)
      super(
        title: String(title),
  ...
  class Base
    ...
    def rendered_template(template, notification_data)
